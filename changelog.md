...
- Fixed `</sequence/>` print bug in the WSDL generators.
...
- Improved generation of schema `<extension>` definitions to allow XML validation in the presence of class overriding.
...
- Updated dom.cpp.
- Changed wsdl2h `std::string*` to `std::string` generation (to avoid double pointers `std::string**`). To obtain old behavior, add entry `xsd:string = | std::string* | std::string*` to typemap.dat.
...
---
- Added xs:redefine support to wsdl2h.
...
- Fixed C++ mapping of xs:sequence maxOccurs>1.
...
- Added JSON serialization support in place of XML-RPC serialization, see the XML-RPC content in the samples folder.
...
- Replaced C lib functions `memcpy`, `memmove`, `strcpy`, `strcat`, `strncpy`, `strncat`, `sscanf`, `sprintf` by more secure alternatives when available on installation platform.
...
- Fixed missing `<restriction>` tag in soapcpp2 WSDL/XSD output.
